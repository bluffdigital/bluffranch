...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (!code) {
            return NextResponse.json({ error: 'Missing authorization code' }, { status: 400, headers });
        }
        ...
        return NextResponse.json({ message: 'Refresh token received. Add to .env.local and restart.' }, { headers });
    } catch (error) {
        console.error('Error in OAuth callback:', error);
        return NextResponse.json({ error: 'Failed to process OAuth callback' }, { status: 500, headers });
    }
}